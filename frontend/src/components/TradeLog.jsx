  ...
  return (
    <div style={{ backgroundColor: '#1e1e1e', padding: '20px', borderRadius: '10px', border: '1px solid #333' }}>
      ...
      <div className="custom-scroll" style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '400px', overflowY: 'auto' }}>
        {logs.length === 0 && <div style={{ color: '#555', fontSize: '0.8rem' }}>Aguardando...</div>}
        {logs.map((log, i) => (
          <div key={i} style={{ fontSize: '0.75rem', padding: '10px', borderRadius: '6px', backgroundColor: '#161616', borderLeft: `4px solid ${log.includes('🟢') ? '#00ff88' : '#ff4444'}`, color: '#eee' }}>
            {log}
          </div>
        ))}
      </div>
