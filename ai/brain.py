...
# 🟢 Recebe os parâmetros do painel React
def preparar_dados_e_treinar(symbol, ai_depth, ai_agressivity, use_ema, learning_rate, estimators, fee_pct):
    try:
        ...
        if '1m' not in dados_completos or len(dados_completos['1m']) < 50:
            return {"error": "Dados insuficientes no BD."}
        ...
        # 🟢 Alvo agora usa a taxa informada pelo painel
        taxa_corretagem = fee_pct / 100 
        ...
        modelo = XGBClassifier(
            ...
            max_depth=ai_depth,
            scale_pos_weight=ai_agressivity, 
            random_state=42,
            eval_metric='logloss'
        )
...
def predict(symbol):
    ...
    learning_rate = float(request.args.get('lr', 0.05))
    estimators = int(request.args.get('est', 100))
    fee_pct = float(request.args.get('fee', 0.1))
    
    return jsonify(preparar_dados_e_treinar(symbol.upper(), ai_depth, ai_agressivity, use_ema, learning_rate, estimators, fee_pct))

if __name__ == '__main__':
    print("🚀 Cérebro HFT Inteligente rodando na 5000...")
